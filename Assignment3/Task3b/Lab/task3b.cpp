...
int main(void) 
{
	...
	try {
		...
		// build the program
		if(!build_program(&program, &context, "task3b.cl")) 
		{
		...
		// create a kernel
		kernel = cl::Kernel(program, "task3b");
		...
		std::cout << "Kernel enqueued for horizontal pass." << std::endl;
		std::cout << "--------------------" << std::endl;
		...
		std::cout << "Kernel enqueued for vertical pass." << std::endl;
		std::cout << "--------------------" << std::endl;
